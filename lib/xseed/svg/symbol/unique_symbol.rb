...
module Xseed
  module Svg
    module Symbol
      # Represents an XML Schema xs:unique identity constraint
      # Unique constraints ensure values are unique within a scope
      class UniqueSymbol < Base
        attr_reader :selector, :fields

        def initialize(name:, xsd_node:)
          super(name: name, type: "unique", xsd_node: xsd_node)
