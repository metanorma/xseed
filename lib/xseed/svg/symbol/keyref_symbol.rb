...
module Xseed
  module Svg
    module Symbol
      # Represents an XML Schema xs:keyref identity constraint
      # Keyref constraints reference keys defined elsewhere
      class KeyrefSymbol < Base
        attr_reader :selector, :fields, :refer

        def initialize(name:, xsd_node:)
          super(name: name, type: "keyref", xsd_node: xsd_node)
        ...
        # Returns the key or unique constraint that this keyref refers to
        def refer_name
          @refer
        end
        ...
        def calculate_bounds
          # Calculate width based on name and refer
          text_length = @name.length
          text_length += @refer.to_s.length + 4 if @refer # Add " -> "
          calculated_width = (text_length * CHAR_WIDTH) + 20
          @width = [MIN_COMPACT_WIDTH, calculated_width].max
