...
module Xseed
  module Svg
    module Symbol
      # Represents an XML Schema xs:selector element within identity constraints
      # Selector specifies the XPath expression for element selection
      class SelectorSymbol < Base
        attr_reader :xpath

        def initialize(xsd_node:)
          @xpath = extract_xpath(xsd_node)
          super(name: "selector: #{@xpath}", type: "selector", xsd_node: xsd_node)
        end

        private

        # Extracts XPath from selector node
        ...
        # Override calculate_bounds for compact selector height
